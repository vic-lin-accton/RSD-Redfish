							 ...
							 int64 length, int8 *version, uint32 version_len)
{
	...
	int8 out_name[256] = {0};
	int8 out_ver[256] = {0};
	int32 ver_digital_cur[5] = {0};
	int32 ver_digital_new[5] = {0};
	int32 i = 0;
	...
	while (fgets(output, 256, pp) != NULL) {
		...
		if (rc == 2) {
			if (strcmp(out_name, comp_type) == 0) {
				/* Compare the version */
				rc = sscanf(out_ver, "%d.%d.%d.%d.%d", &ver_digital_new[0],
							&ver_digital_new[1], &ver_digital_new[2],
							&ver_digital_new[3], &ver_digital_new[4]);
				if (rc < 3) {
					HTTPD_ERR("version mismatch\n");
					rc = RESULT_UNMATCHED_VERSION;
					goto err;
				}
				rc = sscanf(RSA_RMM_FIRMWARE_VERSION, "%d.%d.%d.%d.%d",
							&ver_digital_cur[0],
							&ver_digital_cur[1],
							&ver_digital_cur[2],
							&ver_digital_cur[3],
				...
				HTTPD_INFO("%d.%d.%d.%d.%d::%d.%d.%d.%d.%d\n", ver_digital_cur[0],
						   ver_digital_cur[1], ver_digital_cur[2],
						   ver_digital_cur[3], ver_digital_cur[4],
...
#if 0
//vic_lin comment out this redirect, not sure why redirect here
//Intel originally need this redirect
#ifdef REST_HTTPS
