...
/*	"/var/run/snmpd.pid",   Use ONL snmpd instead. viclin add 2017.10.16 */
	"/var/run/stunnel.pid",
	"/var/run/lldpd.pid" 
};
...
static int need_restart_rmm(void)
{
	...
	for (i = 0; i < total; i++) {
		...
		{		    
		    if (i == (total-1)) /*last one check for lldpd monitor */
		    {

		    	rmm_log(INFO, "lldpd is not alive... \n");
		    	sleep(1);  /* since sometime lldpd need to restart to reset location id */		    	
		        if (is_process_alive(pid_files[i])) /* check lldpd again */
			    alive_process++;	
			else
			    rmm_log(INFO, "lldpd still is not alive... \n");	    	
		    }
	        }
	}
