...
//vic add for drawer psu power	
	if (jrpc_get_named_param_value(req, JRPC_DRAWER_PSUPWR, JSON_INTEGER, &power) == JSONRPC_SUCCESS) {
		if (power != 0) {
			/* save power */
			rc = libdb_attr_set_int(DB_RMM, node_id, DRAWER_PSUPWR_STR, 0x0, power, SNAPSHOT_NEED_NOT, LOCK_ID_NULL);
			if (rc == -1) {
				rmm_log(ERROR, "memdb set drawer power fail\n");
				return -1;
			}
