...
int assetd_drawer_present64(int64 *present, char *uuid)
{
	jrpc_req_pkg_t req_pkg = {};
	int64 type = DZ_TYPE;
	int64 present64 = *present;
...
int assetd_drawer_psupwr_change(int64 *power, char *uuid)
{
	jrpc_req_pkg_t req_pkg = {};
	int64 type = DRAWER_TYPE;

	fill_pkg_hdr(&req_pkg, &type, uuid, NULL);
	fill_param(&req_pkg, JRPC_DRAWER_PSUPWR, power, JSON_INTEGER);
