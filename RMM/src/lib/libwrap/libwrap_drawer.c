...
//vic add
	drawer_member->psu_power =
		(uint32)get_db_info_num(DB_RMM, drawer_node_id, DRAWER_PSUPWR_STR);	
//vic end		
...
//vic add
result_t libwrap_get_drawer_psu_power_by_node_id(memdb_integer drawer_node_id, uint32 *pwr) {
    drawer_member_t drawer;
    int8 host = 0;
    result_t rc = RESULT_OK;
...
result_t libwrap_pack_drawer_to_json(json_t *output, drawer_member_t *drawer_member, struct rest_uri_param *param)
{
	...
	if (oem != NULL) {
		...
		if (rsa != NULL) {
			...
			add_json_integer(rsa, RMM_JSON_DRAWER_POWER_CONSUME, drawer_member->power_consumption); //vic add
