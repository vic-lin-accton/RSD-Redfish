...
#else
        /* "MaxPSUsSupported": 0 */
	add_json_integer(output, RMM_JSON_MAX_PSUS_SUPPORT, 0);        
#endif
...
static uint32 atn_4610_poutmax()
{
	     uint32 psu1_pwr=0,psu2_pwr=0;
	     uint32 psu1_ok=0,psu2_ok=0;
	     char   newline[80];
             FILE*  fd = popen("cat /sys/devices/1803b000.i2c/i2c-1/i2c-8/8-0058/psu_mfr_pout_max", "r");              
             if (NULL != fd)
             {
             	
                while((fgets(newline, 256, fd)) != NULL) {
                	sscanf(newline, "%u", &psu1_pwr);
                }
                pclose(fd);
             }
             fd = popen("cat /sys/devices/1803b000.i2c/i2c-1/i2c-8/8-0059/psu_mfr_pout_max", "r");              
             if (NULL != fd)
             {
             	
                while((fgets(newline, 256, fd)) != NULL) {
                	sscanf(newline, "%u", &psu2_pwr);
                }
                pclose(fd);
             }
             fd = popen("cat /sys/devices/1803b000.i2c/i2c-1/i2c-8/8-0058/psu_p_out", "r");              
             if (NULL != fd)
             {
             	
                while((fgets(newline, 256, fd)) != NULL) {
                	sscanf(newline, "%u", &psu1_ok);
                }
                
                pclose(fd);
             }
             fd = popen("cat /sys/devices/1803b000.i2c/i2c-1/i2c-8/8-0059/psu_p_out", "r");              
             if (NULL != fd)
             {
             	
                while((fgets(newline, 256, fd)) != NULL) {
                	sscanf(newline, "%u", &psu2_ok);
                }
                pclose(fd);
             }             
             ...
             if (psu1_ok && psu2_ok)
             {
             	//will show log in Restd 
             	rmm_log(INFO, "PSU1 power max is %d \n",  psu1_pwr);
             	rmm_log(INFO, "PSU2 power max is %d \n",  psu2_pwr);
                return ((psu1_pwr+psu2_pwr)/1000);
             }
             else if (psu1_ok)
             {
             	return (psu1_pwr/1000);
             }
             else if (psu2_ok)
             {
             	return (psu2_pwr/1000);
             }
             else
                return 0;	
}

static uint32 atn_4610_psuout()
//Accton Add 4610 switch psu1/psu2 power into rack total power
{
    	     uint32 psu1_pwr=0,psu2_pwr=0;
    	     char   newline[80];
//#if 1   	
             ...
             if (NULL != fd)
             {
                ...
                while((fgets(newline, 256, fd)) != NULL) {
                	sscanf(newline, "%s", platform);
                }
                pclose(fd);
             }    	 
             if (strstr(platform,"x86-64-accton-as5912-54x"))
             {
                 strcpy(psu1_loc,"cat /sys/bus/i2c/devices/10-0058/psu_p_out");
                 strcpy(psu2_loc,"cat /sys/bus/i2c/devices/11-005b/psu_p_out");	 
             }
             else if (strstr(platform,"arm-accton-as4610"))
             {    
             	 strcpy(psu1_loc,"cat /sys/devices/1803b000.i2c/i2c-1/i2c-8/8-0058/psu_p_out");
                 strcpy(psu2_loc,"cat /sys/devices/1803b000.i2c/i2c-1/i2c-8/8-0059/psu_p_out");
             }
                                
             fd = popen(psu1_loc, "r");     
             //FILE*  fd = popen("cat /sys/devices/1803b000.i2c/i2c-1/i2c-8/8-0058/psu_p_out", "r");              
             if (NULL != fd)
             {
             ...
             if (psu1_pwr || psu2_pwr)
             {
             	//will show log in Restd 
             	rmm_log(INFO, "PSU1 power consumption is %d loc is %s\n",  psu1_pwr, psu1_loc);
             	rmm_log(INFO, "PSU2 power consumption is %d loc is %s\n",  psu2_pwr, psu2_loc);
                return ((psu1_pwr+psu2_pwr)/1000);
             }
             else
                return 0;
...
static uint32 get_power_zone_psu_power(memdb_integer cm_id) {
    ...
    for (i = 0; i < dzones_num; i++) {
        ...
        if (curr_cm_id == cm_id) {
            ...
            for(j = 0; j < drawers_num; j++) {
                ...
                rmm_log(DBG, "Attempting to read Drawer %d psu power...\n", drawer_id);
                rc = libwrap_get_drawer_psu_power_by_node_id(drawer_id, &d_pwr);
                if (RESULT_OK != rc) {
                    rmm_log(DBG, "Cannot read Drawer %d psu power\n", drawer_id);
                    continue;
                }
                p_pwr += d_pwr;
                rmm_log(DBG, "Drawer %d psu power is %d\n", drawer_id, d_pwr);
            }
        }
    }
    libdb_free_node(dzones);
...
static result_t get_pzone_by_idx(uint32 pzone_idx, struct pzone_member *pzone_member)
{
	...
	pzone_member->tot_power_cap += atn_4610_poutmax(); //add 4610 itself
#ifdef INTEL_RACK
	pzone_member->tot_power_production =
		(uint32)get_db_info_num(DB_RMM, pzone_node_id, PZONE_TT_PWR_PROD_STR);
#else
	pzone_member->tot_power_production = get_power_zone_psu_power(cm_node_id);
