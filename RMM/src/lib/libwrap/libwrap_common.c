...
int get_drawer_pres_from_cm(int8 *pres)
{
	...
	int32 i,j;
	int32 cm_lid = 0;
	uint64 drawer_pres = 0;; //vic said int32->uint64
	int32 tmp = 0;
	int32 dr_number = 0;
	...
	for (i = 0; i < subnode_num; i++) {
		...
		for (j = 0; j < dr_number; j++) {
			...
			drawer_pres = ( (uint64) 1 << (dr_lid - 1 + ((cm_lid - 1) * 4)))|drawer_pres;
			printf("cm[%d] dr_number[%d] dr_idx[%d] drawer_pres[%llx], node_id[%llx]\n", cm_lid, dr_number, dr_lid, drawer_pres, dr_sub_node[j].node_id);
		}
		free(dr_sub_node);
	}

        //watch this vic said, 8-> MAX_DRAWER_PRESENCE
	for (i = 0; i < MAX_DRAWER_PRESENCE ; i++) {
		strncat_s(pres, RSIZE_MAX_STR, ((0x800000000000 == (0x800000000000 & (drawer_pres << i)))? "1" : "0"), 1);
	}
