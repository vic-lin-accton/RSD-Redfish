...
//	{1000,			false,	process_actions,				NULL},	
	{3000,			false,	get_drawer_present, 		NULL},
	};
	...
	{500,			false,	get_drawers_alert,			get_drawers_alert_rsp},
        ...
        {1000,			false,	get_drawers_rack_location_id,	NULL},
        {1000,			false,	set_drawers_rack_location_id,   NULL},
...
void am_run(am_node_t **nptrs, int32 cnt)
{
	...
	for (index = 0; index < cnt; index++) {
		//rmm_log(INFO, "1. cm index %d start\n",index);
		cm = nptrs[index];
		if (is_cm_idle(cm)) {
...
int32 get_next_availible_asset64(int32 pos, int64 present, int32 max)
{
	int32 i = 0;
...
static char rack_location_id[MAX_LOCATION_ID_LEN] = {'1','\0'};
static unsigned int location_id_change = 0;
void process_next_func(am_node_t *cm)
{
	...
	if (1 == location_id_change)
	{	
		rmm_log(INFO,"rack location id changed to %s\n",rack_location_id);
		location_id_change=0;
	}

	//rmm_log(INFO, "32. func index %d to do \n",data->func_idx);
...
static void free_node(am_node_t *node) {
	if (NULL != node) {
		if (NULL != node->priv_data) {
			free(node->priv_data);
		}
		free(node);
	}
...
void record_rack_location_id(char* location_id)
{
#if 0    
	char cmd[512] = {0};
	char *sys_path = "export PATH=/sbin/:/bin/:/usr/local/sbin:/usr/bin";
	char *lldp_loc_id ="/usr/local/sbin/lldpcli configure lldp custom-tlv add oui 00,00,e8 subtype 1 oui-info 4c,6f,63,61,74,69,6f,6e,49,44,3a";
	char sys_location_id[300]; //for lldpcli to configure location id , suppose length > 103+96*2
	int i=0;
	
	strncpy_safe(rack_location_id, location_id, MAX_LOCATION_ID_LEN, strlen(location_id));
	

    	//snprintf_s_s(cmd, sizeof(cmd), "%s && hostname %s ", sys_path,location_id);
	//status = system(cmd);	
	
    	//snprintf_s_s(cmd, sizeof(cmd), "%s && lldpd -S \"Rackscale:Edgecore RMM Switch AS4610-54P\" -C \!\* -I port* ", sys_path);
	//status = system(cmd);
	
snprintf(cmd, sizeof(cmd), "killall lldpd ; /usr/local/sbin/lldpd");
system(cmd);
strncpy_safe(sys_location_id,lldp_loc_id,300,strlen(lldp_loc_id));
for (i=0;i<strlen(location_id);i++)
{
     int ascii_val=location_id[i];
     char ascii_hex_str[4];
     snprintf(ascii_hex_str,4,",%2x",ascii_val);
     printf("location_id %c ascii_val 0x%02x ascii_hex_str %s\n", location_id[i],ascii_val,ascii_hex_str);
     strncat_s(sys_location_id,300,ascii_hex_str,strlen(ascii_hex_str));
}
printf("sys_location_id is %s\n",sys_location_id);
snprintf_s_s(cmd, sizeof(cmd), "%s", sys_location_id);
system(cmd);	
...
//	snprintf_s_s(cmd, sizeof(cmd), "%s && killall lldpd ", sys_path);
//	system(cmd);

#if 0	
    	snprintf_s_ss(cmd, sizeof(cmd), "%s && hostname %s ", sys_path,location_id);
	system(cmd);	
	snprintf_s_s(cmd, sizeof(cmd), "%s && lldpd -S 'Rackscale:Edgecore RMM Switch AS4610-54P' -C \\!\\*  ", sys_path);
	system(cmd);
#else

      	snprintf_s_ss(cmd, sizeof(cmd), "%s && lldpcli configure system chassisid %s ", sys_path,location_id);
	system(cmd);
      	snprintf_s_s(cmd, sizeof(cmd), "%s && lldpcli update", sys_path);
	system(cmd);	
	printf("location id changed to %s\n",location_id);
	location_id_change = 1;
//	sleep(1);
//	lldpd_restart=1;
...
char* get_rack_location_id()
{
    return rack_location_id;
}
