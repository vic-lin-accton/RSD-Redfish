...
static void present_change64(int64 type, int64 old_present, int64 present, int max, char *uuids, char *parent_uuid, int cm_lid)
{
	int64 i = 0;
	bool old_bit = 0;
	bool new_bit = 0;
...
//vic add start
int notify_pzone_total_power_cap_change(am_node_t *pz, int cap)
{
	pzone_data_t *pz_data = (pzone_data_t *)pz->priv_data;
	int64 pz_lid = 1;
	int64 data = cap;
...
//viclin add start
#if 0
int notify_pzone_total_power_consum_change(am_node_t *pz, int pwr_consum)
{

	cm_data_t *cm_data = (cm_data_t *)pz->parent->priv_data;
	pzone_data_t *pz_data = (pzone_data_t *)pz->priv_data;
	int64 data = pwr_consum;
	
	return assetd_pzone_total_power_consum_change(&data, pz_data->uuid[pz_data->pos]);
}
#endif
//viclin add end
...
int notify_drawer_present_change64(am_node_t *dz, int64 present64)
{
	...
	int64 old_present = dz_data->present64;
	int64 new_present = present64;
	int cm_lid = cm_data->loc_id;

	present_change64(type, old_present, new_present, MAX_DRAWER_NUM, (char *)drawers_data->uuid, dz_data->uuid[dz_data->pos], cm_lid);

	return assetd_drawer_present64(&new_present, dz_data->uuid[dz_data->pos]);
}
...
int notify_drawer_psupwr_change(am_node_t *drawers, unsigned int power)
{
	drawers_data_t *drawers_data = (drawers_data_t *)drawers->priv_data;
	am_node_t *cm = get_cm_node(drawers);
	int64 data = power;

	return assetd_drawer_psupwr_change(&data, drawers_data->uuid[drawers_data->pos]);
}
