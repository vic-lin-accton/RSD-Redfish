...
handler_tbl_t action_handler_tbl[] = {
	...
	[AMACTION_SET_RACK_PUID] = {false, NULL, 0, {0}},
        [AMACTION_SET_RACK_LOCATION_ID] = {false, NULL, 0, {0}}
};
...
int set_rack_location_id(jrpc_req_pkg_t *req_pkg, json_t *resp, void *args) {
    struct jipmi_msg ipmi_req = {};
    unsigned int host = 0;
    int64 error_code = 0;
    char* location_id;
    ...
    if (get_array_elem(req_pkg, JRPC_RACK_LOCATION_ID, JSON_STRING, &location_id) != 0)
        return -1;

    record_rack_location_id(location_id);
...
int post_psu_by_node_id(jrpc_req_pkg_t *req_pkg, json_t *resp, void *args)
{
	...
	unsigned int  host = 0, host1 = 0, host2 = 0;
	int64 error_code = 0;
	int psu_lid;
