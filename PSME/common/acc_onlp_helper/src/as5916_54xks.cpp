...
int As5916_54xks::get_port_tx_status(int port)
{
    try
    {
...
void As5916_54xks::set_port_tx_status(int port, bool tx_status)
{
    try
    {
        std::lock_guard<std::mutex> lock{m_data_mutex};
...
int As5916_54xks::get_psu_info_by_(int psuid, Psu_Content id)
{
    try
    {
        int ii = 0;
