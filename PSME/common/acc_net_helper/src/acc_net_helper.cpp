...
std::string RFKafkaEntry::get_current_time()
{
    std::array<char, BUFFER_SIZE> buffer;
    buffer.fill(0);
    time_t rawtime;
...
void RFKafkaEntry::set_Kaffa_entry(std::string& event_type,std::string& sensor_type,std::string& Serverity, std::string& message_content, int sensor_id)
{
    char res[BUFFER_SIZE]={};
    char cmd[BUFFER_SIZE]={};
	
    get_current_time();
    std::ifstream m_ifile(m_kafka_tmp_path);
    ...
    if(m_ifile.good())
    {
        ...
        m_ifile.close();
        std::ofstream m_ofile(m_kafka_tmp_path);
        ...
        m_ofile << NewLog;
        m_ofile.close();		
        sprintf(cmd, "Kafka_agent.py -m \"`cat %s`\"", m_kafka_tmp_path.c_str());
        m_help_tools.exec_shell(cmd, res);
        printf("kafka result[%s]\r\n", res);
    }
    else
    {
        std::ofstream m_ofile(m_kafka_tmp_path);
