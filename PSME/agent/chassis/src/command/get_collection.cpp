...
namespace {
    ...
    void process_switch(const Collection& collection, const std::string& uuid,
        ...
        if(enums::CollectionType::EthernetSwitchPorts == collection.get_type()) {
            response_add_subcomponents(NetworkComponents::get_instance()->
...
//Step 7.
            ...
            response_add_subcomponents(ChassisComponents::get_instance()->
                get_fan_manager().get_keys(uuid), response);
        }
...
/*		
			...
			std::cerr << "==PowerZones CollectionType process_manager== " << std::endl;
            response_add_subcomponents(ChassisComponents::get_instance()->
                get_power_zone_manager().get_keys(uuid), response);
        }
*/
/*Nick Added End  : */		
        else {
...
void process_system(const Collection& collection, const std::string& uuid,
    ...
    void get_collection(const GetCollection::Request& request, GetCollection::Response& response) {
        ...
        if (CommonComponents::get_instance()->get_module_manager().entry_exists(uuid)) {
            ...
            const EthernetSwitchPort& port = NetworkComponents::get_instance()->
            ...
            const EthernetSwitch& sw = NetworkComponents::get_instance()->
